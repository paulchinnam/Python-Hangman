import random, time
videoGames = ["roblox", "fortnite", "pokemon", "zelda", "warframe", "splatoon", "residentevil", "hayday", "mariokart", "spiderman"]
movies = ["avengers", "interstellar", "inception", "tenet", "aquaman", "tarzan", "terminator", "alien", "shazam", "coco"]
userGuesslist = []
userGuesses = []
playGame = True
category = ""
...
while True:
    ...
    while True:
        if category.upper() == 'M':
            secretWord = random.choice(movies)
            break
        elif category.upper() == 'V':
            secretWord = random.choice(videoGames)
            break
        else:
            category = input("Please select a valid categary: M for Movies / V for Video Games; X to exit")
    ...
    if playGame:
        ...
        if continueGame.upper() == 'Y':
            category = input("Please select a valid categary: M for Movies / V for Video Gamess")
            userGuesslist = []
            userGuesses = []
            playGame = True
        else:
